...
HRESULT DeviceContext::initFromDevice(ID3D11Device* device)
{
  ...
  destroy();
  device->GetImmediateContext(&m_deviceContext); // AddRef interno
...
HRESULT DeviceContext::attach(ID3D11DeviceContext* ctx)
{
  ...
  destroy();
  m_deviceContext = ctx;
  m_deviceContext->AddRef(); // retenemos nuestra propia referencia
  return S_OK;
}
...
{
  if (m_deviceContext) m_deviceContext->OMSetRenderTargets(NumViews, ppRenderTargetViews, pDepthStencilView);
}
...
{
  if (m_deviceContext) m_deviceContext->IASetVertexBuffers(StartSlot, NumBuffers, ppVertexBuffers, pStrides, pOffsets);
}
...
// ---------- Update / Draw ----------
void DeviceContext::UpdateSubresource(ID3D11Resource* pDstResource, UINT DstSubresource,
  const D3D11_BOX* pDstBox, const void* pSrcData, UINT SrcRowPitch, UINT SrcDepthPitch)
{
  if (m_deviceContext) m_deviceContext->UpdateSubresource(pDstResource, DstSubresource, pDstBox, pSrcData, SrcRowPitch, SrcDepthPitch);
}

void DeviceContext::DrawIndexed(UINT IndexCount, UINT StartIndexLocation, INT BaseVertexLocation)
{
  if (m_deviceContext) m_deviceContext->DrawIndexed(IndexCount, StartIndexLocation, BaseVertexLocation);
}
