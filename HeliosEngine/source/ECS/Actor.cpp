...
Actor::Actor(Device& device) {
	...
	// Inicializar Constant Buffer para la matriz de mundo
	...
	// Llama al método virtual de inicialización temprana
	awake();

	// Inicializa el Sampler State para el filtrado de texturas de este actor
	...
	// (Pendientes para futuras etapas del motor)
	// ----------------------------------------------------
...
Actor::render(DeviceContext& deviceContext) {
	...
	// Dile a DirectX que dibujaremos usando triángulos
	deviceContext.IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	// Update buffer and render all components (meshes)
	for (unsigned int i = 0; i < m_meshes.size(); i++)
	{
		// Carga la geometría en el pipeline
		...
		// Limpieza por mesh (evita que la textura de un modelo afecte a otro que no tiene textura)
		...
		// else: se queda null para evitar fallos de lectura

		deviceContext.DrawIndexed(m_meshes[i].m_numIndex, 0, 0);
	}
}

// Método optimizado específicamente para dibujar la geometría del entorno sin afectar al pipeline estándar
void
Actor::renderForSkybox(DeviceContext& deviceContext) {
	deviceContext.IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	// Renderiza la geometría del cubo (generalmente m_meshes solo tendrá 1 elemento aquí)
	for (unsigned int i = 0; i < m_meshes.size(); i++) {
		m_vertexBuffers[i].render(deviceContext, 0, 1);
		m_indexBuffers[i].render(deviceContext, 0, 1, false, DXGI_FORMAT_R32_UINT);

		// Ejecuta la orden de dibujo basándose en el número de índices del cubo
		deviceContext.DrawIndexed(m_meshes[i].m_numIndex, 0, 0);
	}
}
