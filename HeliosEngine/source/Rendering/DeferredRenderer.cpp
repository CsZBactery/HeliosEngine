...
#include "Rendering/Mesh.h"
 ...
 // ======================================================================================
 // HELPERS DE ACCESO (Hacks técnicos para puentear encapsulación)
 // ======================================================================================
namespace {
...
// ======================================================================================
HRESULT DeferredRenderer::init(Device& device) {
    ...
    hr = m_disabledDepthStencil.init(device, false, D3D11_DEPTH_WRITE_MASK_ZERO, D3D11_COMPARISON_ALWAYS);
    if (FAILED(hr)) return hr;
...
void DeferredRenderer::resize(Device& device, unsigned int width, unsigned int height) {
...
// ======================================================================================
// BUCLE PRINCIPAL DE RENDERIZADO (FRAME FLOW)
// ======================================================================================
void DeferredRenderer::render(DeviceContext& deviceContext, const Camera& camera, RenderScene& scene, EditorViewportPass& viewportPass) {
...
void DeferredRenderer::buildQueues(RenderScene& scene, const Camera& camera) {
    (void)camera;
    m_opaqueQueue.clear();
    m_transparentQueue.clear();
...
void DeferredRenderer::updatePerFrame(const Camera& camera, const RenderScene& scene, DeviceContext& deviceContext) {
...
void DeferredRenderer::updateLightMatrices(const Camera& camera, const RenderScene& scene) {
...
void DeferredRenderer::renderSceneToTarget(DeviceContext& deviceContext, RenderScene& scene, EditorViewportPass& targetPass, bool applyShadows) {
...
void DeferredRenderer::bindGBufferTargets(DeviceContext& deviceContext, ID3D11DepthStencilView* depthStencilView) {
...
void DeferredRenderer::bindFinalTarget(DeviceContext& deviceContext, ID3D11RenderTargetView* renderTargetView, ID3D11DepthStencilView* depthStencilView) {
    ID3D11RenderTargetView* renderTargets[1] = { renderTargetView };
    deviceContext.OMSetRenderTargets(1, renderTargets, depthStencilView);
    deviceContext.OMSetBlendState(m_opaqueBlendState, m_blendFactor, 0xffffffff);
}

void DeferredRenderer::clearDeferredSRVs(DeviceContext& deviceContext) {
    ID3D11ShaderResourceView* nullViews[8] = {};
    deviceContext.PSSetShaderResources(0, 8, nullViews);
}

// ======================================================================================
// PASADA DE GEOMETRÍA (LLENADO DE G-BUFFER)
// ======================================================================================
void DeferredRenderer::renderGeometryPass(DeviceContext& deviceContext) {
...
void DeferredRenderer::renderGeometryObject(DeviceContext& deviceContext, const RenderObject& object) {
...
// ======================================================================================
// PASADA DE ILUMINACIÓN (COMPOSICIÓN FINAL)
// ======================================================================================
void DeferredRenderer::renderLightingPass(DeviceContext& deviceContext) {
    clearDeferredSRVs(deviceContext);
...
void DeferredRenderer::renderSkyboxPass(DeviceContext& deviceContext, RenderScene& scene) {
    if (!scene.skybox) return;
    scene.skybox->render(deviceContext);
}

void DeferredRenderer::renderTransparentPass(DeviceContext& deviceContext) {
    m_perFrameBuffer.render(deviceContext, 0, 1, true);
    if (m_applyShadows && m_shadowDepthSRV.m_textureFromImg) {
        deviceContext.PSSetShaderResources(6, 1, &m_shadowDepthSRV.m_textureFromImg);
    }
...
void DeferredRenderer::renderForwardObject(DeviceContext& deviceContext, const RenderObject& object, RenderPassType passType) {
...
// ======================================================================================
void DeferredRenderer::renderShadowPass(DeviceContext& deviceContext) {
    ...
    for (const RenderObject* object : m_opaqueQueue) {
        if (!object || !object->castShadow) continue;
        renderShadowObject(deviceContext, *object);
    }
}

void DeferredRenderer::renderShadowObject(DeviceContext& deviceContext, const RenderObject& object) {
...
// ======================================================================================
// GESTIÓN DE RECURSOS (G-BUFFER & SHADOWS)
// ======================================================================================
HRESULT DeferredRenderer::createShadowResources(Device& device) {
...
HRESULT DeferredRenderer::createGBufferResources(Device& device, unsigned int width, unsigned int height) {
    m_gBufferEmissiveAlphaRTV.destroy();
    m_gBufferEmissiveAlphaSRV.destroy();
    m_gBufferEmissiveAlphaTexture.destroy();
    m_gBufferWorldAoRTV.destroy();
    ...
    HRESULT hr = createGBufferTarget(device, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, m_gBufferAlbedoMetallicTexture, m_gBufferAlbedoMetallicSRV, m_gBufferAlbedoMetallicRTV);
    if (FAILED(hr)) return hr;

    hr = createGBufferTarget(device, width, height, DXGI_FORMAT_R16G16B16A16_FLOAT, m_gBufferNormalRoughnessTexture, m_gBufferNormalRoughnessSRV, m_gBufferNormalRoughnessRTV);
    if (FAILED(hr)) return hr;

    hr = createGBufferTarget(device, width, height, DXGI_FORMAT_R32G32B32A32_FLOAT, m_gBufferWorldAoTexture, m_gBufferWorldAoSRV, m_gBufferWorldAoRTV);
    if (FAILED(hr)) return hr;

    return createGBufferTarget(device, width, height, DXGI_FORMAT_R16G16B16A16_FLOAT, m_gBufferEmissiveAlphaTexture, m_gBufferEmissiveAlphaSRV, m_gBufferEmissiveAlphaRTV);
}

HRESULT DeferredRenderer::createGBufferTarget(Device& device, unsigned int width, unsigned int height, DXGI_FORMAT format, Texture& texture, Texture& srv, RenderTargetView& rtv) {
    HRESULT hr = texture.init(device, width, height, format, D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE, 1, 0);
    if (FAILED(hr)) return hr;
...
HRESULT DeferredRenderer::createLightingResources(Device& device) {
...
HRESULT DeferredRenderer::createFullScreenQuad(Device& device) {
    MeshComponent mesh;
    mesh.m_vertex = {
...
HRESULT DeferredRenderer::createBlendStates(Device& device) {
...
ID3D11BlendState* DeferredRenderer::resolveBlendState(const Material* material) const {
    if (!material) return m_opaqueBlendState;
    if (material->getDomain() != MaterialDomain::Transparent) return m_opaqueBlendState;
