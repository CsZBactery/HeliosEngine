...
// Variables globales para el control del Gizmo
...
namespace {
    ...
    const char* GetActorTypeLabel(EU::TSharedPointer<Actor> actor) {
        if (actor.isNull()) return "Actor";
        auto lightComponent = actor->getComponent<LightComponent>();
        if (!lightComponent.isNull()) return GetLightTypeLabel(lightComponent->getLightData().type);
        if (!actor->getComponent<MeshRendererComponent>().isNull()) return "Static Mesh Actor";
        if (!actor->getComponent<Transform>().isNull()) return "Empty Actor";
        return "Actor";
    }

    ImVec4 GetActorTypeColor(EU::TSharedPointer<Actor> actor) {
        if (actor.isNull()) return ImVec4(0.45f, 0.47f, 0.52f, 1.0f);
        auto lightComponent = actor->getComponent<LightComponent>();
        if (!lightComponent.isNull()) return ImVec4(0.92f, 0.68f, 0.22f, 1.0f);
        if (!actor->getComponent<MeshRendererComponent>().isNull()) return ImVec4(0.24f, 0.50f, 0.92f, 1.0f);
    ...
    void DrawInspectorComponentChips(bool hasTransform, bool hasMeshRenderer, bool hasLight) {
        if (hasTransform) DrawInspectorPill("Transform", ImVec4(0.18f, 0.50f, 0.28f, 1.0f));
        if (hasMeshRenderer) {
            if (hasTransform) ImGui::SameLine();
            DrawInspectorPill("Renderer", ImVec4(0.22f, 0.42f, 0.76f, 1.0f));
        }
        if (hasLight) {
            if (hasTransform || hasMeshRenderer) ImGui::SameLine();
...
void GUI::outliner(const std::vector<EU::TSharedPointer<Actor>>& actors) {
    ...
    for (int i = 0; i < static_cast<int>(actors.size()); ++i) {
        ...
        if (!filter.PassFilter(filterLabel.c_str())) continue;
...
void GUI::editTransform(Camera& cam, Window& window, EU::TSharedPointer<Actor> actor) {
    ...
    float gizmoRotation[3] = { RadToDeg(rot[0]), RadToDeg(rot[1]), RadToDeg(rot[2]) };
    ...
    if (mCurrentGizmoOperation == ImGuizmo::SCALE) activeGizmoMode = ImGuizmo::LOCAL;
