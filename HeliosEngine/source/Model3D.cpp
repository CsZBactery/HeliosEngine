...
// Incluye optimizaciones avanzadas de Caché Binario y Matemáticas de Tangentes.
// ======================================================================================
...
// ======================================================================================
// SISTEMA DE CACHÉ GLOBAL Y HELPERS BINARIOS (Espacio de nombres anónimo)
// ======================================================================================
namespace {
    ...
    // Caché en RAM para no cargar el mismo modelo dos veces
    std::unordered_map<std::string, ModelCacheEntry> g_modelCache;

    // Helper para comparar fechas de modificación de archivos (Windows API)
    ...
    // Helpers para escribir y leer strings en binario
...
bool Model3D::load(const std::string& path) {
    ...
    // 1. Revisar si ya está en la memoria RAM
    ...
    // 2. Si no, lo inicializamos/procesamos
...
bool Model3D::init() {
    ...
    // 1. Intentar cargar desde el caché binario de disco (Carga ultra rápida)
    ...
    // 2. Si no hay caché binario válido, procesar archivo de texto pesado (OBJ/FBX)
    ...
    // 3. Guardar en memoria y generar el archivo de caché binario en el disco
    m_meshes = loadedMeshes;
    g_modelCache[m_filePath] = ModelCacheEntry{ m_meshes, textureFileNames };
    SaveBinaryCache(cachePath);

    const std::wstring modelPathW(m_filePath.begin(), m_filePath.end());
    MESSAGE("ModelLoader", "Init", L"Loaded model '" << modelPathW << L"' in " << elapsedMs << L" ms. Meshes: " << m_meshes.size())
        return true;
}
...
void Model3D::unload() {
    ...
    // FIX ANTICRASH: Ya no llamamos Destroy() aquí para el FBX. 
    // Ya se limpió en LoadFBXModel después de usarlo. 
    // Hacerlo aquí causaría un Access Violation (Double-Free).
    lScene = nullptr;
    lSdkManager = nullptr;
...
std::vector<MeshComponent> Model3D::LoadFBXModel(const std::string& filePath) {
    ...
    if (InitializeFBXManager()) {
        ...
        if (!lImporter) {
            ...
            if (lScene) { lScene->Destroy(); lScene = nullptr; }
            if (lSdkManager) { lSdkManager->Destroy(); lSdkManager = nullptr; }
            return loadedMeshes;
        }
        ...
        // Convertir coordenadas e interpolar triangulación
        ...
        if (lRootNode) {
            ...
            // FIX ANTICRASH: Limpieza inmediata de memoria FBX tras éxito
            if (lScene) { lScene->Destroy(); lScene = nullptr; }
            if (lSdkManager) { lSdkManager->Destroy(); lSdkManager = nullptr; }
            ...
            // FIX ANTICRASH: Limpieza de memoria FBX en caso de fallo
            if (lScene) { lScene->Destroy(); lScene = nullptr; }
            if (lSdkManager) { lSdkManager->Destroy(); lSdkManager = nullptr; }
...
void Model3D::ProcessFBXMesh(FbxNode* node) {
    ...
    // Calcular tangentes matemáticamente si faltan
...
std::vector<MeshComponent> Model3D::LoadOBJModel(const std::string& filePath) {
    ...
    auto parseFaceVertex = [&](const std::string& token, int positionCount, int texcoordCount, int normalCount) -> ObjIndex {
        ObjIndex result{};
        ...
        const std::string texcoordToken = (firstSlash == std::string::npos) ? std::string() :
    ...
    while (std::getline(file, line)) {
        ...
        if (command == "v") {
            ...
            if (currentGroupName.empty()) currentGroupName = "default";
            currentMesh.name = currentGroupName;
        }
            ...
            std::string token;
            while (stream >> token) {
                const ObjIndex objIndex = parseFaceVertex(token, static_cast<int>(positions.size()), static_cast<int>(texcoords.size()), static_cast<int>(normals.size()));

                auto it = currentMesh.vertexLookup.find(objIndex);
                if (it == currentMesh.vertexLookup.end()) {
                    SimpleVertex vertex{};
                    if (objIndex.position >= 0 && objIndex.position < static_cast<int>(positions.size())) vertex.Position = positions[objIndex.position];
                    if (objIndex.texcoord >= 0 && objIndex.texcoord < static_cast<int>(texcoords.size())) vertex.TextureCoordinate = texcoords[objIndex.texcoord];
                    else vertex.TextureCoordinate = EU::Vector2(0.0f, 0.0f);

                    if (objIndex.normal >= 0 && objIndex.normal < static_cast<int>(normals.size())) vertex.Normal = normals[objIndex.normal];
                    else vertex.Normal = EU::Vector3(0.0f, 1.0f, 0.0f);
...
// ======================================================================================
// FUNCIONES DE CACHÉ BINARIO (.wvmesh)
// ======================================================================================
...
bool Model3D::IsBinaryCacheUpToDate(const std::string& sourcePath, const std::string& cachePath) const {
    ULONGLONG sourceWriteTime = 0;
    ULONGLONG cacheWriteTime = 0;

    if (!GetFileWriteTime(sourcePath, sourceWriteTime)) return false;
    if (!GetFileWriteTime(cachePath, cacheWriteTime)) return false;
...
bool Model3D::LoadBinaryCache(const std::string& cachePath) {
    ...
    for (uint32_t i = 0; i < textureCount; ++i) {
        ...
        if (!ReadString(stream, textureName)) return false;
        loadedTextures.push_back(std::move(textureName));
    }

    for (uint32_t i = 0; i < meshCount; ++i) {
        MeshComponent mesh;
        if (!ReadString(stream, mesh.m_name)) return false;
...
bool Model3D::SaveBinaryCache(const std::string& cachePath) const {
    ...
    for (const std::string& textureName : textureFileNames) {
        if (!WriteString(stream, textureName)) return false;
    }

    for (const MeshComponent& mesh : m_meshes) {
        if (!WriteString(stream, mesh.m_name)) return false;
